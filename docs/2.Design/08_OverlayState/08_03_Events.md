...
- Các phím tắt chuyển công cụ annotation (`P`, `L`, `A`, `R`, `C`, `M`, `T`, `B`, `I`) thường được UI xử lý trước bằng `e.Key` (phím vật lý) để tránh bị bộ gõ tiếng Việt chặn, sau đó gửi `SelectTool tool` vào state machine. Xem thêm `11_05_InputHandling.md`, mục 3.2.
...
- Loại công cụ: Pencil, Line, Arrow, Rectangle, Circle, Marker, Text, Pixelate, Icon.
