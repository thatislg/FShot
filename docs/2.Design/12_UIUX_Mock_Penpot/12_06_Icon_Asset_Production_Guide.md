...
- Geometry: Ultra-rounded edges, 2-head proportion, tilted at [45 độ hoặc đứng thẳng], centered inside 26x26 px safe zone on a 32x32 px canvas.
...
- Composition: Centered, isolated on pure white background (#FFFFFF), square 1:1 aspect ratio, 512x512 px or larger, flat color separation, clean vector sticker style, zero realistic textures.
- Export: PNG with pure white background, high resolution, ready for vectorization to SVG path data.
```
...
- **Xuất:** 1:1 aspect ratio, **512×512 px** PNG nền trắng, sau đó vector hóa thành SVG 32×32 viewBox.

```
[STYLE MASTER SPECIFICATION] A single chibi arrow tool icon for a 32x32px toolbar. A chubby, short, curved golden arrow pointing upwards to the right, centered in a 26x26px safe zone. The arrow head has soft rounded corners with a cute white specular highlight dot. Thick bold deep-brown outline, soft warm clay shading. Flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single cute chubby pencil icon for a 32x32px toolbar. Stubby short wooden pencil body in coral-red color, centered in a 26x26px safe zone, kawaii smiling face with two tiny dot eyes and pink blush cheeks on the barrel. Sharp but rounded wooden nib drawing a tiny squiggle line below. Thick smooth outlines, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single chubby highlighter marker icon for a 32x32px toolbar. Fat, rounded pastel-yellow marker body centered in a 26x26px safe zone, with a wide slanted chisel tip. Glossy highlight on the side, bold clean dark outline, soft clay texture, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single straight line drawing tool icon for a 32x32px toolbar. A clean, straight rounded rod centered in a 26x26px safe zone, with completely rounded pill-shaped ends in pastel mint green color, with tiny white star sparkles. Thick bold outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single rounded rectangle tool icon for a 32x32px toolbar. A thick hollow rounded square frame centered in a 26x26px safe zone, with chubby pill-shaped borders and puffy circular corners in pastel blue, bold smooth dark outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single hollow circle tool icon for a 32x32px toolbar. A chubby doughnut-shaped ring centered in a 26x26px safe zone, with a hollow center in vibrant pastel pink, soft specular reflection on top-left, thick smooth dark outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single pixelate mosaic tool icon for a 32x32px toolbar. A neat 3x3 grid centered in a 26x26px safe zone, composed of 9 tiny squishy rounded square jelly candies in alternating pastel colors (coral pink, mint green, and baby blue). Cute sticker vector, bold outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single blur tool icon for a 32x32px toolbar. A fluffy, puffy chubby little cloud shape centered in a 26x26px safe zone, in soft lavender and white, emitting tiny soft bubbles around it, bold dark rounded outlines, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single step counter badge icon for a 32x32px toolbar. A puffy circular speech bubble centered in a 26x26px safe zone, in pastel baby blue with a tiny cute tail, containing a bold, rounded, white number '1' inside. Sparkling highlights, thick bold outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single pin tool icon for a 32x32px toolbar. A chubby pushpin centered in a 26x26px safe zone, with a round bulbous plastic head in sky blue, an adorable smiling kawaii face on the pin head, and a short, stubby rounded silver needle tip. Thick smooth outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single text tool icon for a 32x32px toolbar. A super chubby bold capital letter 'A' centered in a 26x26px safe zone, with swollen, pillow-like rounded limbs, wearing a tiny happy face expression, soft drop shadow, bold outlines, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single crop selection tool icon for a 32x32px toolbar. A rounded squircle selection box centered in a 26x26px safe zone, with dashed lines and cute curly looped handles at all four corners, pastel cyan and mint color scheme, flat clean vector look, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single full-screen capture tool icon for a 32x32px toolbar. A chubby retro desktop computer monitor centered in a 26x26px safe zone, with an ultra-rounded screen, tiny antenna on top, cute aesthetic, thick smooth outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single copy to clipboard icon for a 32x32px toolbar. Two overlapping chubby rounded sheets of paper centered in a 26x26px safe zone, with folded corners, held together by a cute colorful paperclip, soft sticker style, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single save icon for a 32x32px toolbar. A vintage 3.5-inch floppy disk centered in a 26x26px safe zone, reimagined as a cute squishy toy with rounded corners, mint macaron green plastic casing, white label sticker, bold outlines, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single undo tool icon for a 32x32px toolbar. A chubby counter-clockwise curved arrow centered in a 26x26px safe zone, shaped like a plump horseshoe with rounded edges, soft pastel lilac color, thick dark stroke, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
[STYLE MASTER SPECIFICATION] A single cancel cross icon for a 32x32px toolbar. A chubby, puffed-up 'X' shape centered in a 26x26px safe zone, with bulbous rounded tips, soft strawberry red color, bold smooth dark outline, flat vector colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, ready for SVG vectorization.
```
...
```
A single kawaii selection crop tool icon for a 32x32px app UI toolbar, rounded dashed square border in pastel mint and cyan, curly loop nodes at four corners, centered in 26x26 safe zone, 2D flat vector sticker style, isolated on a pure white background, 1:1 aspect ratio, 512x512px, minimal and clean.
```

```
A single pixelate tool icon for a 32x32px app UI toolbar, a 3x3 grid composed of 9 cute rounded squishy jelly candy blocks in pastel peach, mint, and sky blue colors, centered in 26x26 safe zone, 2D vector illustration, isolated on pure white background, 1:1 aspect ratio, 512x512px, clean vector lines.
```

```
A single counter step bubble icon for a 32x32px app UI toolbar, a puffy round speech bubble in pastel baby blue with a white bold rounded number '1' inside, tiny cute star sparkle accent, centered in 26x26 safe zone, clean vector sticker style, isolated on a pure white background, 1:1 aspect ratio, 512x512px.
```

```
A single cute stubby pencil icon for a 32x32px toolbar, fat and short red crayon pencil with a sharp tip, centered in 26x26 safe zone, kawaii smiling face with tiny blush dots on the wooden body, bold dark outlines, flat pastel colors, 2D vector graphic, isolated on pure white background, 1:1 aspect ratio, 512x512px.
```

```
A single kawaii fat highlighter marker icon for a 32x32px toolbar, pastel yellow and black slanted chisel tip, chubby rounded body, centered in 26x26 safe zone, clean sticker vector style, soft pastel colors, isolated on pure white background, 1:1 aspect ratio, 512x512px, minimal clean lines.
```

```
A single cute pushpin icon for a 32x32px app UI toolbar, chubby pastel blue pinhead with an adorable kawaii smiling face, slanted metallic tip, centered in 26x26 safe zone, clean 2D vector sticker style, flat shading, isolated on pure white background, 1:1 aspect ratio, 512x512px.
```

```
A single cute vintage 3.5-inch floppy disk icon for save action on a 32x32px toolbar, pastel mint green plastic shell with rounded corners, white label sticker, chubby cartoon aesthetic, centered in 26x26 safe zone, 2D clean vector icon, isolated on pure white background, 1:1 aspect ratio, 512x512px.
```
...
3. Sinh ảnh **PNG 512×512 px, tỷ lệ 1:1, nền trắng (#FFFFFF)**.
4. Vector hóa bằng Vectorizer.AI / SVGcode.
5. Làm sạch trong Inkscape: xóa nền trắng, resize page to selection, simplify path, xuất **Optimized SVG** với viewBox 32×32 px.
6. Nhúng geometry path data vào Avalonia UI hoặc lưu vào thư mục Assets cho MSIX.
